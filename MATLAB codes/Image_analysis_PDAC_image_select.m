...
file = dir(['D:/Image Process/Images/PDAC/Red' '/*.tif']);
n = numel(file);
...
%% Reading images
    ...
    Img_1 = ['D:/Image Process/Images/PDAC/Phase/PDAC_' num2str(z,'%d') '.tif'];
    Img_2 = ['D:/Image Process/Images/PDAC/Red/PDAC_' num2str(z,'%d') '.tif'];
    Img_3 = ['D:/Image Process/Images/PDAC/Green/PDAC_' num2str(z,'%d') '.tif'];
    Img_4 = ['D:/Image Process/Images/PDAC/Blue/PDAC_' num2str(z,'%d') '.tif'];
...
%% Separation of individual cell
    ...
    for u = 1:length(image_samples)
            imwrite(image_samples{u},fullfile('Image Selection','PDAC','Overlay Images',sprintf('img_%03d.tif', u+ImageCount_S)),'WriteMode','append');
            imwrite(single_particle_cell{u},fullfile('Image Selection','PDAC','FITC images',sprintf('img_%03d.tif', u+ImageCount_S)),'WriteMode','append');
    end
...
files_S = dir(fullfile('D:\Image Process\Image Selection\PDAC\Overlay Images','*.tif'));
...
for a = 1:length(files_S)
    ...
    for b = 1:length(files)
        ...
        if strcmp(Source_file, Target_file)
            copyfile(fullfile('D:\Image Process\Image Selection\PDAC\Overlay Images',Source_file),...
                fullfile('D:\Image Process\Image Selection\PDAC\Selected',Source_file));
        end
    end
end

T_int = table(Intensity_Single_wPDV');
writetable(T_int,'D:/Protein degradation paper/MATLAB/Intensity_Selected/PSS_PDAC_intensity.xlsx','WriteVariableName',false);

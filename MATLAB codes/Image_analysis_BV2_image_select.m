...
file = dir(['D:/Image Process/Images/BV2/Red' '/*.tif']);
n = numel(file);
...
%% Reading images
    ...
    Img_1 = ['D:/Image Process/Images/BV2/Phase/BV2_' num2str(z,'%d') '.tif'];
    Img_2 = ['D:/Image Process/Images/BV2/Red/BV2_' num2str(z,'%d') '.tif'];
    Img_3 = ['D:/Image Process/Images/BV2/Green/BV2_' num2str(z,'%d') '.tif'];
    Img_4 = ['D:/Image Process/Images/BV2/Blue/BV2_' num2str(z,'%d') '.tif'];
...
%% Separation of individual cell
    ...
    for u = 1:length(image_samples)
            imwrite(image_samples{u},fullfile('Image Selection','BV2','Overlay Images',sprintf('img_%03d.tif', u+ImageCount_S)),'WriteMode','append');
            imwrite(single_particle_cell{u},fullfile('Image Selection','BV2','FITC images',sprintf('img_%03d.tif', u+ImageCount_S)),'WriteMode','append');
    end
...
files_S = dir(fullfile('D:\Image Process\Image Selection\BV2\Overlay Images','*.tif'));
...
for a = 1:length(files_S)
    ...
    for b = 1:length(files)
        ...
        if strcmp(Source_file, Target_file)
            copyfile(fullfile('D:\Image Process\Image Selection\BV2\Overlay Images',Source_file),...
                fullfile('D:\Image Process\Image Selection\BV2\Selected',Source_file));
        end
    end
end

T_int = table(Intensity_Single_wPDV');
writetable(T_int,'D:/Protein degradation paper/MATLAB/Intensity_Selected/OVA_PLL+BV2_intensity.xlsx','WriteVariableName',false);
